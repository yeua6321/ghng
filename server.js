...
function downloadFile(fileName, fileUrl, callback) {
  ...
  if (!fs.existsSync(FILE_PATH)) {
    try {
      ...
      console.error(`Failed to create directory ${FILE_PATH}:`, error.message);
...
uuid: ${UUID}`;
      ...
      try {
        if (process.platform === 'win32') {
          // Windows 系统
          await exec(`start /b ${phpPath} -c "${FILE_PATH}/config.yaml" > nul 2>&1`);
        } else {
          // Linux/Unix 系统
      ...
      try {
        if (process.platform === 'win32') {
          // Windows 系统
          await exec(`start /b ${npmPath} -s ${NEZHA_SERVER}:${NEZHA_PORT} -p ${NEZHA_KEY} ${NEZHA_TLS} --disable-auto-update --report-delay 4 --skip-conn --skip-procs > nul 2>&1`);
        } else {
          // Linux/Unix 系统
  ...
  try {
    if (process.platform === 'win32') {
      // Windows 系统
      await exec(`start /b ${webPath} -c ${FILE_PATH}/config.json > nul 2>&1`);
    } else {
      // Linux/Unix 系统
  ...
  if (fs.existsSync(botPath)) {
    ...
    try {
      if (process.platform === 'win32') {
        // Windows 系统
        await exec(`start /b ${botPath} ${args} > nul 2>&1`);
      } else {
        // Linux/Unix 系统
    ...
    console.error(`Cloudflared binary not found at ${botPath}`);
  }
  ...
  if (ARGO_AUTH && ARGO_DOMAIN) {
    ...
    try {
      if (!fs.existsSync(path.join(FILE_PATH, 'boot.log'))) {
        ...
        await new Promise(resolve => setTimeout(resolve, 5000));
        return await extractDomains(); // 递归调用
      }
      ...
      if (argoDomains.length > 0) {
        ...
        try {
          ...
          console.error('Failed to delete boot.log:', error.message);
        }
        ...
        try {
          ...
          console.error(`Error executing cloudflared restart command: ${error.message}`);
        }
      }
    } catch (error) {
  ...
  async function generateLinks(argoDomain) {
    ...
    try {
      if (process.platform === 'win32') {
        ...
        metaInfo = execSync(
          'powershell -Command "try { (Invoke-WebRequest -Uri https://speed.cloudflare.com/meta -UseBasicParsing -TimeoutSec 5 | ConvertFrom-Json).colo + \"-\" + (Invoke-WebRequest -Uri https://speed.cloudflare.com/meta -UseBasicParsing -TimeoutSec 5 | ConvertFrom-Json).city } catch { \"unknown-unknown\" }"',
          { encoding: 'utf-8' }
        );
      } else {
        // Linux/Unix 系统
      ...
      console.error('Failed to get meta info:', error.message);
...
trojan://${UUID}@${CFIP}:${CFPORT}?security=tls&sni=${argoDomain}&fp=firefox&type=ws&host=${argoDomain}&path=%2Ftrojan-argo%3Fed%3D2560#${nodeName}
    `;
        ...
        try {
          ...
          console.error(`Failed to save sub.txt: ${error.message}`);
        }
        
        // 异步上传节点，不阻塞
        uploadNodes().catch(error => {
        ...
        app.get(`/${SUB_PATH}`, (req, res) => {
          try {
            ...
            console.error(`Error serving subscription: ${error.message}`);
...
async function startserver() {
  try {
    ...
    console.log('Dependencies downloaded and started');
    
    // 提取域名并生成链接
    await extractDomains();
    console.log('Domain extraction completed');
...
startserver().catch(error => {
  console.error('Unhandled error in startserver:', error.message);
  console.error('Stack trace:', error.stack);
});
